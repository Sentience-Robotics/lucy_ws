...
For example, if you wanted to add an `rqt_graph` standalone tool, you would append this to the `packages` array:
...
- **Standalones:** The script spins up a new tmux window named after the package's `id` and executes its command. Alternatively, if a complex command object is provided, it executes the explicit `start` and `stop` shell commands in the background.

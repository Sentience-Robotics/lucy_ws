...
- **Modifier start hooks:** A modifier carrying `lifecycle_hooks.start` also gets its own window, named by `lifecycle_window`. Selecting `real` therefore gives you two windows: `core` for the ROS stack and `embedded` for the firmware bridge. The bridge waits for `core` to create its shared memory, reads the `node_name` from it, and runs `firmwares/sim`. Opening the board's serial port needs `config/udev/99-lucy-rp2040.rules` (install with `pixi run setup-udev` or `./scripts/setup_udev.sh`) — without it the bridge dies with `Permission denied` on `/dev/ttyACM0`, since `dialout` membership never reaches an already-running tmux server.
